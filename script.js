...
const movies = [
   {
    ...
    episodes: [
      { name: "Filme Completo", link: "https://1024terabox.com/s/1MqxhHeDtHLatWS1PDbtvNg" },
    ],
  },
   {
    title: "Superman 2025",
...
function showDetails(index) {
  ...
  movie.episodes.forEach((ep) => {
    ...
    // Criar link que ABRE NO PLAYER INTERNO (em vez de abrir nova aba)
...
// === Player interno: abrir/fechar no overlay ===
function openPlayer(link) {
  const playerSection = document.getElementById('playerSection');
  const playerFrame = document.getElementById('playerFrame');
  const detailsEl = document.getElementById('details');

  // tenta extrair ID do Google Drive e usar preview; senão usa link direto
  const match = link.match(/\/d\/([^\/]+)/);
  if (match && match[1]) {
    const fileId = match[1];
    playerFrame.src = `https://drive.google.com/file/d/${fileId}/preview`;
  } else {
